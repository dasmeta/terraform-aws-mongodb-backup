...
STORAGE=${S3_BUCKET}
MIGRATE_CMD="aws s3 cp /backup/${NEWEST_BACKUP_FILE}.zip s3://${STORAGE}/${NEWEST_BACKUP_FILE}.zip"

if [[ -n "${NEWEST_BACKUP_FILE}" ]]; then
    echo "=> Compressing ${NEWEST_BACKUP_FILE} file"
    echo "=> The latest mongo dump is: ${NEWEST_BACKUP_FILE}"
    cd /backup
    zip -r /backup/"${NEWEST_BACKUP_FILE}.zip" ./"${NEWEST_BACKUP_FILE}"
    ...
    rm -f /backup/"${NEWEST_BACKUP_FILE}.zip"
    echo "=> Removed!"
